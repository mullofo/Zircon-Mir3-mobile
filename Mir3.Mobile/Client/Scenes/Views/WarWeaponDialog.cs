                ...
                if (GameScene.Game.MapControl.Objects != null)
                {
                    if (Map?.PetData != null)
                    {
                        if (CEnvir.LibraryList.TryGetValue(LibraryFile.UI1, out MirLibrary lib))
                        {
                            ...
                            if (image != null)
                                PresentMirImage(image.Image, this, new Rectangle(panel.DisplayArea.X, panel.DisplayArea.Y, (int)(image.Width * percent), image.Height), Color.White, panel, zoomRate: ZoomRate, uiOffsetX: UI_Offset_X);
                        }
                    }
                }
            };
