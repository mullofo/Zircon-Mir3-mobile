...
namespace Client.Scenes.Views
{
    ...
    public sealed class CommunicationDialog : DXWindow
    {
        ...
        public CommunicationDialog()
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                row.Location = new Point(5, row.Size.Height * i + 2);
                row.MouseClick += (o, e) =>
                {
                    SelectedFriend = FriendList[i + FriendScrollBar.Value];
                ...
                row.FreeDrag += Friend_DoFreeDrag;
                FriendRows[i] = row;
            }
            Panel.FreeDrag += Friend_DoFreeDrag;
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                Rows[index].FreeDrag += Mail_DoFreeDrag;
            }
            panel.FreeDrag += Mail_DoFreeDrag;
        ...
        public void Mail_DoFreeDrag(object sender, TouchEventArgs e)
        {
        ...
        public void Friend_DoFreeDrag(object sender, TouchEventArgs e)
        {
            int value = (int)e.Delta.Y;
            FriendScrollBar.Value -= value;
        }
    ...
    public sealed class FriendRow : DXControl
    {
        ...
        public FriendRow()
        {
            Size s = TextRenderer.MeasureText(DXManager.Graphics, "字", new Font(Config.FontName, CEnvir.FontSize(10F)));
            Size = new Size(240, s.Height);
            DrawTexture = true;
            Border = true;
