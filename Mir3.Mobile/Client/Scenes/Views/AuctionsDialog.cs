...
namespace Client.Scenes.Views
{
    ...
    public sealed class AuctionsDialog : DXWindow
    {
        ...
        public AuctionsDialog()
        {
            ...
            rowControl.FreeDrag += SearchScrollBar_DoFreeDrag;

            for (int i = 0; i < SearchRows.Length; i++)
            {
                var row = new AuctionsRow
                {
                    Parent = rowControl,
                    Location = new Point(0, i * 34),
                };
                row.MouseClick += (o, e) => { SelectedRow = row; };
                row.FreeDrag += SearchScrollBar_DoFreeDrag;
                row.MouseEnter += (o, e) => { GameScene.Game.MouseItem = row.NewAuctionInfo?.Item; };
        ...
        public void SearchScrollBar_DoFreeDrag(object sender, TouchEventArgs e)
        {
            int value = (int)e.Delta.Y;
            SearchScrollBar.Value -= value / 34;
        }
        #endregion
