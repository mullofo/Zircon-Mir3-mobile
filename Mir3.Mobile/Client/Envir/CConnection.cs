...
namespace Client.Envir
{
    ...
    public sealed class CConnection : BaseConnection
    {
        ...
        {
            byte[] clientHash = System.Text.Encoding.Default.GetBytes(string.Format("{0}.{1}", Config.VersionName, Config.VersionCode));
            byte[] clientSystemDBHash = new byte[1];
            string file = Path.Combine(CEnvir.MobileClientPath, @"Data/ClientSystem.db");
        ...
        public void Process(G.GoodVersion p)  //正确版本
        {
            ...
            //服务区列表
            Config.Server1Name = p.Server1Name;
            if (p.PlayCount < 5)
            {
                Config.Server1Name += " (良好)";
            }
            else if (p.PlayCount < 20)
            {
                Config.Server1Name += " (繁忙)";
            }
            else
            {
                Config.Server1Name += " (满员)";
            }
        ...
        public void Process(S.ClientNameChanged p)   //客户端区服名称
        {
            ...
            Config.ClientName = p.ClientName;
            //古墓任务序号
        ...
        public void Process(S.NewAccount p)  //新建账号
        {
            ...
            switch (p.Result)
            {
                ...
                case NewAccountResult.AlreadyExists:
                    ...
                    //login.AccountBox.EMailTextBox.SetFocus();
                    login.ShowMsg("E-Mail地址已被使用".Lang());
                ...
                case NewAccountResult.Success:
                    ...
                    //login.AccountBox.Clear();
                    //DXMessageBox.Show("你的账号已成功创建\n请按照发送到你的电子邮件说明进行激活。".Lang(), "账号创建".Lang());
                    break;
            }
        }
        ...
        public void Process(S.RequestPasswordReset p)  //请求密码重置
        {
            ...
            switch (p.Result)
            {
                ...
                case RequestPasswordResetResult.ResetDelay:
                    ...
                    login.ShowMsg($"你不能这么快就请求另一个重置密码".Lang() + "\n" +
                                            $"下次可重置".Lang() + $": {expiry}\n" +
                                            $"Cconnection.Duration".Lang(Math.Floor(p.Duration.TotalHours).ToString("#,##0"), p.Duration.Minutes, p.Duration.Seconds));
                ...
                case RequestPasswordResetResult.Success:
                    ...
                    login.ShowMsg("重置密码请求成功".Lang() + "\n" +
                                     "请查看你的电子邮件以获取进一步的说明".Lang());
                    login.NativeUI.HidePopWindow();
                    login.NativeUI.ShowLayout = true;
                    break;
            }
        ...
        public void Process(S.Login p)  //登录
        {
            ...
            switch (p.Result)
            {
                ...
                case LoginResult.AlreadyLoggedIn:
                    ...
                    login.ShowMsg("账号目前正在使用中请稍后再试".Lang());
                    break;
                ...
                case LoginResult.AlreadyLoggedInAdmin:
                    ...
                    login.ShowMsg("账号目前正由管理员使用".Lang());
                    break;
                case LoginResult.Success:   //登录结果成功
                    login.NativeUI.ShowLayout = false;
                    login.NativeUI.HidePopWindow();
                    ...
                    if (Config.RememberDetails)
                    {
                        ...
                        Config.RememberedEMail = login.NativeUI.Account;
                        Config.RememberedPassword = login.NativeUI.Pwd;
                        ConfigReader.Save();
                    }
                    ...
                    var serverScence = new ServerScene(CEnvir.GameSize)
                    {
        ...
        public void Process(S.StartGame p)  //开始游戏
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var login = new LoginScene(CEnvir.GameSize);
        ...
        public void Process(S.ObjectRemove p)  //对象删除
        {
            ...
            if (GameScene.Game.MonsterBox.Object != null && GameScene.Game.MonsterBox.Object.ObjectID == p.ObjectID)
                GameScene.Game.MonsterBox.Object = null;
        ...
        public void Process(S.ObjectStruck p)   //击打对象
        {
            ...
            foreach (MapObject ob in GameScene.Game.MapControl.Objects)
            {
                ...
                {
                    //记录最后一个攻击玩家
                    var lastob = GameScene.Game.MapControl.Objects.FirstOrDefault(x => x.ObjectID == p.AttackerID);
                    if (lastob != null && lastob.Race == ObjectType.Player)
                    {
                        //MapObject.User.LastAttackPlayer = lastob;

                        GameScene.Game.EnemyMemberBox.AttackedMembers.Add(lastob.ObjectID);
                        GameScene.Game.EnemyMemberBox.Background.Visible = true;
                        //只保留7个最后攻击者
                        if (GameScene.Game.EnemyMemberBox.AttackedMembers.Count == 8)
                            GameScene.Game.EnemyMemberBox.AttackedMembers.Remove(GameScene.Game.EnemyMemberBox.AttackedMembers.First());
                    }
                ...
                ob.Struck(p.AttackerID, p.Element, AttackObImage);
            ...
            foreach (MapObject ob in GameScene.Game.MapControl.Objects)
            {
                ...
                if (ob == MapObject.User)
                {
                    ...
                    if (!GameScene.Game.Observer)
                        GameScene.Game.TownReviveButton.Visible = true;
#endif
                }
        ...
        public void Process(S.ObjectHarvested p)  //收获对象 捕猎
        {
            foreach (MapObject ob in GameScene.Game.MapControl.Objects)
            {
                ...
                GameScene.Game.MapControl.Harvest = false;
        ...
        public void Process(S.ChangeAttackMode p)             //收到攻击模式封包
        {
            ...
            GameScene.Game.ReceiveChat(p.Mode.Lang(), MessageType.System);
        }
        ...
        {
#if Mobile
            if (GameScene.Game?.NetherworldGateLabel != null)
            {
                if (p.NetherworldCloseTime <= CEnvir.Now)
                {
                    if (p.NetherworldCloseTime != DateTime.MinValue)
                    {
                        ...
                        //Netherworld gate is closed. Last open: {}
                        GameScene.Game.NetherworldGateLabel.Text = $"异界之门已关闭, 最近开启时间 {lastOpenTime}";
                    }
                }
                else
                {
                    //门开启中
                    // Netherworld gate is opening at {}, close time {}
                    string closeTime = p.NetherworldCloseTime.ToString("HH:mm");
                    GameScene.Game.NetherworldGateLabel.Text = $"异界之门已开启, 坐标({p.NetherworldLocation.X}:{p.NetherworldLocation.Y}), 关闭时间 {closeTime}";
                }
            }
#else
        ...
        public void Process(S.SellCharacterSearch p)
        {
            ...
            GameScene.Game.AccountConsignmentBox.TotalPage = (GameScene.Game.AccountConsignmentBox.SearchResults.Count() - 1) / 5 + 1;
