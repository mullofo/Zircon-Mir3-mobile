    ...
    public sealed class MapControl : DXControl
    {
        ...
        protected override void OnClearTexture()
        {
            ...
            if (FLayer.TextureValid)
                DXManager.Sprite.Draw(FLayer.ControlTexture, Color.White.ToRawColorBGRA());
            ...
            DXManager.Sprite.Draw(LLayer.ControlTexture, Color.White.ToRawColorBGRA());
        ...
        private void LoadMap()
        {
            try
            {
                if (!File.Exists(Path.Combine(Config.MapPath, MapInfo.FileName + ".map"))) return;

                DXManager.ForceReworkTextures();
                Cells = new MapCells(Path.Combine(Config.MapPath, MapInfo.FileName + ".map"));
                Width = Cells.Width;
                Height = Cells.Height;
        ...
        public void ProcessInput()
        {
            ...
            if (MapObject.TargetObject != null && !MapObject.TargetObject.Dead && ((MapObject.TargetObject.Race == ObjectType.Monster && string.IsNullOrEmpty(MapObject.TargetObject.PetOwner)) || (CEnvir.Shift || BigPatchConfig.ChkAvertShift)))
            {
                ...
                if (BigPatchConfig.AndroidPlayer)
                {
                    ...
                    if ((User.Class == MirClass.Wizard || User.Class == MirClass.Taoist || MapObject.User.Class == MirClass.Assassin) && BigPatchConfig.AndroidSingleSkill && BigPatchConfig.AndroidSkills != MagicType.None)
                    {
                        ...
                        {
                            //与目标距离
                            float dis = Functions.Distance(MapObject.User.CurrentLocation, MapObject.TargetObject.CurrentLocation);
                            //如果与目标距离小于3
                            if (dis < 3)
                            {
                                //目标方向
            ...
            //跑不停
            if (GameScene.Game.AutoRun && !haselementalhurricane || BigPatchConfig.ChkKeepRunning && !haselementalhurricane)
            {
                ...
                for (int y = minY; y <= maxY; y++)
                {
                    ...
                    for (int x = minX; x <= maxX; x++)
                    {
                        ...
                        if (tile.BackImage != -1)
                        {
                            ...
                            //绘制背景层，既地砖
                            library.Draw(backimage, drawX, drawY, Color.White, false, 1F, ImageType.Image);
                        }
                ...
                if (IsBoat)
                {
                    ...
                    for (int x = 0; x < 10; x++)
                    {
                        for (int y = 0; y < 10; y++)
                        {
                            ...
                            library.Draw(561, Boat[x, y].X, Boat[x, y].Y, Color.White, false, 1f, ImageType.Image);
                        }
                    }
                    if (flag)
                    {
        ...
        public sealed class Light : DXControl
        {
            ...
            protected override void OnClearTexture()
            {
                ...
                if (MapObject.User.Dead)
                {
                    ...
                    {
                        DXManager.Device.Clear(ClearFlags.Target, Color.IndianRed.ToRawColorBGRA(), 0, 0);
                    }
                    return;
                }
                ...
                {
                    DXManager.Device.Clear(ClearFlags.Target, Color.Black.ToRawColorBGRA(), 0, 0);
                ...
                for (int y = minY; y <= maxY; y++)
                {
                    ...
                    for (int x = minX; x <= maxX; x++)
                    {
                        ...
                        DXManager.Sprite.Draw(DXManager.LightTexture, Vector3.Zero, new Vector3(fX, fY, 0), Color.FromArgb(m_stRGBInfo[LightColor].bRed, m_stRGBInfo[LightColor].bGreen, m_stRGBInfo[LightColor].bBlue));
