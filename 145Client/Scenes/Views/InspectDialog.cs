            ...
            if (drawArmour)
            {
                ...
                if (CEnvir.LibraryList.TryGetValue(LibraryFile.EquipEffect_UI, out effectLibrary))
                {
                    ...
                    if (image != null)
                    {
                        ...
                        PresentTexture(image.Image, CharacterTab, new Rectangle(DisplayArea.X + x + image.OffSetX, DisplayArea.Y + y + image.OffSetY, image.Width, image.Height), ForeColour, this, 0, 0, true, 0.8f);
            ...
            if (CEnvir.LibraryList.TryGetValue(LibraryFile.ProgUse, out library))
            {
                ...
                if (drawHair)
                    if (Class == MirClass.Assassin && Gender == MirGender.Female && HairType == 1)
                        library.Draw(1160, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, HairColour, true, 1F, ImageType.Image);
                    ...
                    if (HairType > 0)    //发型 单独处理 不能用自定义特效功能
                    {
                        switch (Class)
                        {
                            ...
                            case MirClass.Taoist:
                                ...
                                {
                                    case MirGender.Male:
                                        library.Draw(60 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, Color.White, true, 1F, ImageType.Image);
                                        library.Draw(60 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, HairColour, true, 1F, ImageType.Overlay);
                                        break;
                                    case MirGender.Female:
                                        library.Draw(80 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, Color.White, true, 1F, ImageType.Image);
                                        library.Draw(80 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, HairColour, true, 1F, ImageType.Overlay);
                                        break;
                                }
                                break;
                            case MirClass.Assassin:
                                ...
                                {
                                    case MirGender.Male:
                                        library.Draw(1100 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, Color.White, true, 1F, ImageType.Image);
                                        library.Draw(1100 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, HairColour, true, 1F, ImageType.Overlay);
                                        break;
                                    case MirGender.Female:
                                        library.Draw(1120 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, Color.White, true, 1F, ImageType.Image);
                                        library.Draw(1120 + HairType - 1, DisplayArea.X + x - 1, DisplayArea.Y + y - 1, HairColour, true, 1F, ImageType.Overlay);
                                        break;
                                }
                                break;
                        }
