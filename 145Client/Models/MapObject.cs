    ...
    public abstract class MapObject
    {
        ...
        public Color NameColour
        {
            get
            {
                ...
                if (GameScene.Game.GuildAlliances.Contains(Title)) return Color.FromArgb(25, 70, 25);   //联盟行会的颜色 墨绿色
        ...
        private void SmoothUpdateFrame()
        {
            ...
            switch (CurrentAction)
            {
                case MirAction.Moving:
                    ...
                    double frame = 1.0 - ((CEnvir.Now - FrameStart).Ticks / 10000L) / 600.0;
                    if (frame < 0.0)
                    {
                        frame = 0.0;
                    }
                    switch (Direction)
                    {
                        case MirDirection.Up:
                            x = 0;
                            y = (int)((CellHeight * MoveDistance) * frame);
                            break;
                        case MirDirection.UpRight:
                            x = -(int)((CellWidth * MoveDistance) * frame);
                            y = (int)((CellHeight * MoveDistance) * frame);
                            break;
        ...
        public void Struck(uint attackerID, Element element)
        {
            ...
            DiyMagicEffect ActSpellMagic = Globals.DiyMagicEffectList.Binding.FirstOrDefault(p => p.MagicType == DiyMagicType.MagicAttackPlayer && p.MagicID == AttackerID);
