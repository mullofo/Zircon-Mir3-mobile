    ...
    public class DXControl : IDisposable
    {
        ...
        public virtual void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (IsMoving)  //正在移动
            {
                ...
                tempPoint.X = Math.Max(tempPoint.X, -DisplayArea.Width + 30);  //往左移剩余20
                tempPoint.X = Math.Min(Parent.Size.Width - 30, tempPoint.X);

                tempPoint.Y = Math.Max(tempPoint.Y, 0);   //不可以移动出上边
                tempPoint.Y = Math.Min(Parent.Size.Height - 30, tempPoint.Y);   //不可以移动出下边
        ...
        public virtual void OnMouseDown(MouseEventArgs e)
        {
            ...
            if (AllowResize)
            {
                ...
                {
                    if (e.Location.X - DisplayArea.Left < ResizeBuffer)
                        ResizeLeft = CanResizeLeft;
                    else if (DisplayArea.Right - e.Location.X < ResizeBuffer)
                        ResizeRight = CanResizeRight;
                }

                if (CanResizeHeight)
                {
                    if (e.Location.Y - DisplayArea.Top < ResizeBuffer)
                        ResizeUp = CanResizeUp;
                    else if (DisplayArea.Bottom - e.Location.Y < ResizeBuffer)
                        ResizeDown = CanResizeDown;
                }
