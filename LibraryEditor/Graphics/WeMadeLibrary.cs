...
namespace LibraryEditor
{
    public class WeMadeLibrary
    {
        ...
        public void Initialize()
        {
            ...
            LMain.form1.toolStripProgressBar.Maximum = Images.Length;
        ...
        private void LoadImageInfo()
        {
            ...
            LoadIndexFile();
            Images = new MImage[_indexList.Count];
        }
        ...
        private void CheckImage(int index)
        {
            ...
            if (Images[index] == null)
            {
                ...
                Images[index] = new MImage(_bReader, _nType);
            }
        ...
        public void ToMLibrary(byte imageTextureType, byte shadowTextureType, byte overlayTextureType, bool crypt)
        {
            ...
            if (fname.StartsWith("Mon-"))
            {
                ...
                if (File.Exists(shadowPath))
                    shadowLibrary = new WeMadeLibrary(shadowPath);
            }
        ...
        public class MImage
        {
            ...
            private byte[][] DecompressWemadeMir3(BinaryReader BReader, short OutputWidth, short OutputHeight, int InputLength)
            {
                ...
                Pixels[0] = new byte[OutputWidth * OutputHeight * 4];
                Pixels[1] = new byte[OutputWidth * OutputHeight * 4];
                        ...
                        switch (FileBytes[OffSet])
                        {
                            ...
                            case 193:  //Solid Colour
                                ...
                                for (int i = 0; i < Count; i++)
                                {
                                    ...
                                    int color = FileBytes[OffSet + 1] << 8 | FileBytes[OffSet];
                                    byte red = (byte)((color & 0xf800) >> 8);
                                    byte green = (byte)((color & 0x07e0) >> 3);
                                    byte blue = (byte)((color & 0x001f) << 3);
                                    Pixels[0][(Y * OutputWidth + x) * 4] = blue;
                                    Pixels[0][(Y * OutputWidth + x) * 4 + 1] = green;
                                    Pixels[0][(Y * OutputWidth + x) * 4 + 2] = red;
                                    Pixels[0][(Y * OutputWidth + x) * 4 + 3] = 255;
                                    OffSet += 2;
                            ...
                            case 195:
                                ...
                                for (int i = 0; i < Count; i++)
                                {
                                    for (int j = 0; j < 2; j++)
                                    {
                                        ...
                                        byte green = (byte)((color & 0x07e0) >> 3);
                                        byte blue = (byte)((color & 0x001f) << 3);
                                        Pixels[j][(Y * OutputWidth + x) * 4] = blue;
                                        Pixels[j][(Y * OutputWidth + x) * 4 + 1] = green;
                                        Pixels[j][(Y * OutputWidth + x) * 4 + 2] = red;
                                        Pixels[j][(Y * OutputWidth + x) * 4 + 3] = 255;
                                    }
                                    OffSet += 2;
                                    if (x >= OutputWidth) continue;
                                    x++;
