...
{
    public sealed class ZirconLibrary
    {
        ...
        public ZirconLibrary(string fileName)
        {
            FileName = fileName;
        ...
        public void ToMLibrary(byte imageTextureType, byte shadowTextureType, byte overlayTextureType, bool crypt)
        {
            ...
            try
            {
                Parallel.For(0, Images.Count, options, i =>
                {
                    ...
                    //bitmap的宽高已经是4的倍数
                    library.Images[i] = new BlackDragonLibrary.MImage(image.Image, image.ShadowImage, image.OverlayImage, imageTextureType, shadowTextureType, overlayTextureType)
                    {
                        Width = image.Width,
                        Height = image.Height,
                        ...
                        Shadow = image.ShadowType,

                        ShadowWidth = image.ShadowWidth,
                        ShadowHeight = image.ShadowHeight,
                        OverlayWidth = image.OverlayWidth,
                        OverlayHeight = image.OverlayHeight,
                    };
                });
            }
        ...
        public sealed class MImage : IDisposable
        {
            ...
            public unsafe MImage(Bitmap image, Bitmap shadow, Bitmap overlay)
            {
                if (image == null)
                {
                    FBytes = new byte[0];
